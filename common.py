...
def command_line_chk():
    ...
    parser.add_argument('-a', '--all', action='store_true', help="run mode All_Data")
    args = parser.parse_args()
    if args.version:
        print("===============================")
        print("DCASE 2020 task 2 baseline\nversion {}".format(__versions__))
        print("===============================\n")
    if args.eval ^ args.dev ^ args.all:
        if args.dev:
            flag = True
        elif args.all:
            flag = 2
...
def select_dirs(param, mode):
    ...
    if mode == 1:
        ...
        logger.info("load_directory <- development + evaluation")
        dir_path = os.path.abspath("{base}/*".format(base=param["all_data_directory"]))
        dirs = sorted(glob.glob(dir_path))
    else:
